...
void update_world(int *world, int width, int height) {
    ...
    int *new_world = calloc(size, sizeof(int));
    if (new_world == NULL) {
        fprintf(stderr, "Error allocating memory for new world\n");
        return;
    }
    ...
    // Copy new_world back to world
    memcpy(world, new_world, size * sizeof(int));
